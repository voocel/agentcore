...
func TestAgentLoop_StopAfterTool(t *testing.T) {
	...
	if endEvent.Summary.StopAfterTool != "commit" {
		t.Fatalf("expected RunSummary.StopAfterTool=commit, got %q", endEvent.Summary.StopAfterTool)
	}
}

func TestAgentLoop_ExclusiveTurnToolSkipsSiblings(t *testing.T) {
	var writeCalls int32
	var enterCalls int32

	writeTool := NewFuncTool("write", "write file", map[string]any{
		"type": "object", "properties": map[string]any{},
	}, func(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
		atomic.AddInt32(&writeCalls, 1)
		return json.Marshal(map[string]string{"status": "wrote"})
	})
	enterTool := NewFuncTool("enter_plan_mode", "enter plan mode", map[string]any{
		"type": "object", "properties": map[string]any{},
	}, func(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
		atomic.AddInt32(&enterCalls, 1)
		return json.Marshal(map[string]string{"status": "entered"})
	})

	events := runTestLoop(t,
		[]AgentMessage{UserMsg("plan and write")},
		AgentContext{Tools: []Tool{writeTool, enterTool}},
		LoopConfig{
			StreamFn: mockStreamFn(Message{
				Role: RoleAssistant,
				Content: []ContentBlock{
					ToolCallBlock(ToolCall{ID: "tc-write", Name: "write", Args: json.RawMessage(`{}`)}),
					ToolCallBlock(ToolCall{ID: "tc-enter", Name: "enter_plan_mode", Args: json.RawMessage(`{}`)}),
				},
				StopReason: StopReasonToolUse,
			}),
			ExclusiveTurnTool: func(name string) bool {
				return name == "enter_plan_mode"
			},
			StopAfterTool: func(name string) bool {
				return name == "enter_plan_mode"
			},
	...
	if got := atomic.LoadInt32(&writeCalls); got != 0 {
		t.Fatalf("write should be skipped when bundled with exclusive tool, got %d calls", got)
	}
	if got := atomic.LoadInt32(&enterCalls); got != 1 {
		t.Fatalf("enter_plan_mode should execute exactly once, got %d calls", got)
	}
	var sawWriteSkipped bool
	for _, ev := range events {
		if ev.Type == EventToolExecEnd && ev.ToolID == "tc-write" && ev.IsError && strings.Contains(string(ev.Result), "exclusive") {
			sawWriteSkipped = true
			break
		}
	}
	if !sawWriteSkipped {
		t.Fatal("expected write sibling to receive exclusive-tool skipped result")
	}
}

func TestAgentLoop_ExclusiveTurnToolDefersStreamingExecution(t *testing.T) {
	var writeCalls int32
	var enterCalls int32
	...
	model := newScriptedStreamModel(streamAssistantToolCalls(
		"planning",
		StopReasonToolUse,
		0,
		ToolCall{ID: "tc-write", Name: "write", Args: json.RawMessage(`{}`)},
		ToolCall{ID: "tc-enter", Name: "enter_plan_mode", Args: json.RawMessage(`{}`)},
	))
	...
	if got := atomic.LoadInt32(&writeCalls); got != 0 {
		t.Fatalf("streamed write should be deferred and skipped, got %d calls", got)
	}
	if got := atomic.LoadInt32(&enterCalls); got != 1 {
		t.Fatalf("streamed enter_plan_mode should execute exactly once, got %d calls", got)
	}
}

func TestAgentLoop_ExclusiveTurnToolRunsFirstExclusiveOnly(t *testing.T) {
	var enterCalls int32
	var exitCalls int32
	...
	exitTool := NewFuncTool("exit_plan_mode", "exit plan mode", map[string]any{
		"type": "object", "properties": map[string]any{},
	}, func(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
		atomic.AddInt32(&exitCalls, 1)
		return json.Marshal(map[string]string{"status": "exited"})
	})

	events := runTestLoop(t,
		[]AgentMessage{UserMsg("bad transition bundle")},
		AgentContext{Tools: []Tool{enterTool, exitTool}},
		LoopConfig{
			StreamFn: mockStreamFn(Message{
				Role: RoleAssistant,
				Content: []ContentBlock{
					ToolCallBlock(ToolCall{ID: "tc-enter", Name: "enter_plan_mode", Args: json.RawMessage(`{}`)}),
					ToolCallBlock(ToolCall{ID: "tc-exit", Name: "exit_plan_mode", Args: json.RawMessage(`{}`)}),
				},
				StopReason: StopReasonToolUse,
			}),
			ExclusiveTurnTool: func(name string) bool {
	...
	if got := atomic.LoadInt32(&enterCalls); got != 1 {
		t.Fatalf("first exclusive tool should execute once, got %d enter calls", got)
	}
	if got := atomic.LoadInt32(&exitCalls); got != 0 {
		t.Fatalf("second exclusive sibling should be skipped, got %d exit calls", got)
	}
	var sawExitSkipped bool
	for _, ev := range events {
		if ev.Type == EventToolExecEnd && ev.ToolID == "tc-exit" && ev.IsError && strings.Contains(string(ev.Result), "exclusive") {
			sawExitSkipped = true
			break
		}
	}
	if !sawExitSkipped {
		t.Fatal("expected second exclusive sibling to receive skipped result")
	}
}
